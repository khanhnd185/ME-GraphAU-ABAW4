...
if __name__=="__main__":
    ...
    img = pil_loader(os.path.join(dataset_path, 'cropped_aligned', args.img))
    img = transform(img)
    img = img.unsqueeze(0)
    ...
    dataset = SAW2(dataset_path, train=False, transform=image_test_saw2(img_size=224), stage = 2)

    index = [i for i, x in enumerate(dataset.data_list) if x[0] == args.img]

    if len(index) > 0:
        index = index[0]
        print('img: ', dataset.data_list[index][0])
        print('va : ', dataset.data_list[index][1])
        print('exp: ', dataset.data_list[index][2])
        print('aus: ', dataset.data_list[index][3])

        i = torch.argmax(yhat_ex, dim=1)
        print('yhat_va: ', yhat_va)
        print('yhat_ex: ', i)
        print('yhat_au: ', yhat_au >= 0.5)
    else:
        print("No image")
